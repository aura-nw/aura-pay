...
class AppLocalizationManager {
  ...
  bool setCurrentLocale(String localeCode) {
    LogProvider.log('Set current locale: $localeCode');
    if (_supportedLocale.containsKey(localeCode)) {
      LogProvider.log('Setting locale to: $localeCode');
      _locale = Locale(localeCode);
      LogProvider.log('Current locale set to: $_locale');
      return true;
    }
    _locale = const Locale('en');
    LogProvider.log('Fallback to default locale: $_locale');
    return false;
  }
  ...
  Future<void> load() async {
    ...
    LogProvider.log('Supported locales: $supportLocales');

    /// Get user setting selected locale
    String? userSelectedLocale = await _localizationUseCase.getSelectedLocale();

    LogProvider.log('User selected locale: $userSelectedLocale');
    ...
    if (userSelectedLocale != null) {
      ...
      LogProvider.log('Applied user selected locale: $userSelectedLocale');
      return;
    }
  ...
  Future<void> updateDeviceLocale(String deviceLocale) async {
    LogProvider.log('Device locale: $deviceLocale');
    LogProvider.log('Is user setting locale: $isUserSettingLocale');
